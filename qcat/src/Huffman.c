...
void decode_MSST19(unsigned char *s, size_t targetLength, node t, int *out, int maxBits)
{
    ...
    unsigned char* lengthTable = (unsigned char*)malloc(tableSize * sizeof(int));
    node* nodeTable = (node*)malloc(tableSize * sizeof(node));
    unsigned int maskTable[maxBits+8];
    int j;
    for(unsigned int i=0; i<tableSize; i++){
        n = t;
        j = 0;
        while(!n->t && j < maxBits){
            unsigned int res = i >> (maxBits - j - 1);
            if((res & 0x00000001) == 0){
                n = n->left;
            }else{
                n = n->right;
    ...
    while(count<targetLength)
	{
        ...
        unsigned int index = currentValue >> (leftBits - maxBits);
        int value = valueTable[index];
        if(value != -1){
			out[count] = value;
			int bitLength = lengthTable[index];
			leftBits -= bitLength;
			unsigned int avoidHeadMask = maskTable[maxBits + 8 - leftBits - 1];
