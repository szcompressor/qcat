...
int main(int argc, char *argv[]){
  ...
  if(argc<4 || argc>7){
    cout<<"Compute Sobolev norm...."<<endl;
    cout<<"Wrong number of arguments. Usage:"<<endl;
    cout<<"./computeSobolevNorm originalDataFile reconstructedDataFile {dimensions}"<<endl;
    cout<<"Dimensions are in reversed order, from fastest to slowest: dim0 dim1 dim2 dim3"<<endl;
    cout<<"In C array representation: arrayName[dim3][dim2][dim1][dim0]"<<endl;
    cout<<"EXAMPLE:"<<endl;
    cout<<"./computeSobolevNorm CLOUD_100x500x500.dat CLOUD_100x500x500.dat.sz.out 500 500 100"<<endl;
    return 0;
  }
  ...
  ifstream lossyFile;
  lossyFile.open(argv[2],ios::in | ios::binary);
  if(!lossyFile.good()){
    cout<<"ERROR: There is a problem with the input file."<<endl;
    cout<<"Trying to read size: ( "<<dim0<<" x "<<dim1<<" x "<<dim2<<" x "<<dim3<<" ) from  file:"<<argv[1]<<endl;
    cout<<"(dim0,dim1,dim2,dim3)"<<endl;
  ...
  double sobolev_s1_p2 = qcat_calc_metric_der_order1_sobolev_p2_float(origData, lossyData, nDims, dim3, dim2, dim1, dim0, &status);
  if(status==0)
    cout<<"sobolev_s1_p2 = "<<sobolev_s1_p2<<endl;
