...
int main(int argc, char * argv[])
{
    ...
    int status = 0;
    char dataType[4];
    char oriFilePath[640], lapFilePath[640];
    
    if(argc < 4)
    {
		printf("Usage: computeLaplacian [datatype (-f or -d)] [original data file (input)] [laplacian matrix file (output)]\n");
		printf("			-f means single precision; -d means double precision\n");
		printf("Example: computeLaplacian -f CLOUD_100x500x500.dat CLOUD_100x500x500_lap.dat 500 500 100\n");
		exit(0);
    }
   
    sprintf(dataType, "%s", argv[1]);
    sprintf(oriFilePath, "%s", argv[2]);
    sprintf(lapFilePath, "%s", argv[3]);
   ...
   size_t nbEle = 0;
    if(strcmp(dataType, "-f")==0) //single precision
	{
		...
		float* lap_data = (float *)malloc(nbEle*sizeof(float));
		
		printf("calcaulting....\n");
		computeLaplacian(ori_data, lap_data, dType, 0, r4, r3, r2, r1);
		
		//writing results
		printf("Writing resulting data to %s\n", lapFilePath);		
		writeData_inBytes(lap_data, dType, nbEle, lapFilePath, &status);
	...
	else
	{
		...
		double* lap_data = (double *)malloc(nbEle*sizeof(double));
